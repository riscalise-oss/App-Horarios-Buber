...
def cargar_datos():
    ...
    # --- CARGAR RESERVAS ESPECIALES ---
    df_res = pd.read_csv(LINK_RESERVAS, header=None, on_bad_lines='skip', engine='python')
    
    return df_o, df_res, espacios

try:
    df_ocupados, df_res, todos_los_espacios = cargar_datos()
    ...
    with tab1:
        ...
        # --- LÓGICA DE RESERVAS ESPECIALES (Cálculo del profesor desplazado) ---
        espacios_reservados_hoy = []
        
        # Buscador inteligente: Lee la fila de Google Sheets buscando el Día y Bloque exactos
        for idx, row in df_res.iterrows():
            fila_lista = [str(x).strip().upper() for x in row.values if pd.notna(x)]
            
            # Si coinciden el Día y el Bloque en esa fila...
            if dia_elegido in fila_lista and (str(bloque_elegido) in fila_lista or f"{bloque_elegido}.0" in fila_lista):
                # Extraemos qué espacio es el que se reservó
        ...
        alertas_ui = []
        # Traducimos tu fórmula =FILTER() a Python
        for e in espacios_reservados_hoy:
            ...
            if not df_esp.empty:
                ...
                if not df_clases.empty:
                    profs = " y ".join([p for p in df_clases['DOCENTES'].dropna().unique() if str(p).upper() != "NAN"])
                    mats = " - ".join([m for m in df_clases['MATERIA'].dropna().unique() if str(m).upper() != "NAN"])
                    alertas_ui.append(f"**{e}** - ⚠️ Desplaza a: {profs} ({mats})")
                else:
                    alertas_ui.append(f"**{e}** - ✅ Libre en horario fijo")
            else:
                alertas_ui.append(f"**{e}** - ✅ Libre en horario fijo")
        ...
        for e in todos_los_espacios:
            # MAGIA: Excluir de la lista verde a los que tienen reserva especial
        ...
        if alertas_ui:
            for alerta in sorted(alertas_ui):
                if "⚠️" in alerta:
                    st.warning(alerta)
                else:
                    st.info(alerta)
